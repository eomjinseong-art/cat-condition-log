...
10초 습관으로 남기고, 병원에는 한 장으로 가져가는 고양이 전용 기록 웹앱입니다.
...
- 더보기에서 CSV 내보내기, 돌봄 일정, 노묘 돌봄(약·재진·검사), 고양이/계정 삭제
...
| `cats` | 이름, 생일, 체중(0.01kg), 사진 URL, 메모, 노묘/만성케어, 돌봄 태그 |
| `logs` | 하루 1행 컨디션(식욕·물·대변·소변·구토·활력·체중·노묘 체크·메모) |
| `reminders` | 약 / 백신 / 구충 / 모래 전체갈이 |
| `medication_plans` / `medication_doses` | 반복 약 스케줄과 「줬어요」 기록 |
| `visits` | 병원·재진 일정 |
| `lab_entries` | 손기록 검사 숫자 |
| `fluid_logs` | 피하수액 용량·자리 |
...
앱 안에서는 물건을 팔지 않습니다. 랜딩(인증 버튼 근처)과 홈 **하단**에만 숨숨마을로 가는 부드러운 안내 카드를 둡니다. 컨디션·약 폼, 모달, 기록 중간에는 넣지 않습니다.

- 안내: [숨숨마을위키](https://b-cat-cpang.vercel.app/wiki/) — 일반적인 돌봄 설명
- 용품: [숨숨마을](https://b-cat-cpang.vercel.app/) — 새 탭(`target="_blank"` `rel="noopener noreferrer"`)

팝업·쿠팡 직접 링크·공격적 구매 유도는 쓰지 않습니다.
