...
export function logsToCsv(cats: PublicCat[], logs: PublicLog[]) {
  ...
  const rows = [...logs]
    ...
    .map((log) => [
      ...
      labelOrDash(log.waterChange, changeLabels),
      labelOrDash(log.urineChange, changeLabels),
      labelOrDash(log.mobility, mobilityLabels),
      labelOrDash(log.nightVocal, nightVocalLabels),
      log.foodNote ?? "",
      log.vomitNote ?? "",
      log.memo ?? "",
    ]);
